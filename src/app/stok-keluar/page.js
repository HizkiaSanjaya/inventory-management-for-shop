...
export default function StokKeluarPage() {
  ...
  const [historyKeluar, setHistoryKeluar] = useState([
    { tanggal: "12 Mei 2026", nama: "Bolpoin Faster Hitam", jumlah: 10 }
  ]);
  ...
  // Ambil data database master barang saat dimuat
  useEffect(() => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!jumlahKeluar || Number(jumlahKeluar) <= 0) {
      alert("Masukkan jumlah kuantitas reduksi stok yang valid!");
      return;
    }

    // 1. CEK VALIDASI KETERSEDIAAN STOK (Mencegah Stok Minus)
    const savedData = localStorage.getItem("database_barang");
    if (savedData) {
      const parsedData = JSON.parse(savedData);
      const barangDipilih = parsedData.find((b) => b.nama === selectedBarang);

      if (barangDipilih && barangDipilih.stok < Number(jumlahKeluar)) {
        alert(`Transaksi Ditolak! Stok "${selectedBarang}" tidak mencukupi. (Sisa Stok: ${barangDipilih.stok})`);
        return;
      }

      // 2. POTONG STOK DI DATABASE UTAMA
      const updatedData = parsedData.map((barang) => 
        barang.nama === selectedBarang 
          ? { ...barang, stok: barang.stok - Number(jumlahKeluar) } 
          : barang
      );
      localStorage.setItem("database_barang", JSON.stringify(updatedData));
    }

    // 3. TAMBAH KE TRANSAKSI RIWAYAT KELUAR
    const transaksiBaru = {
      tanggal: new Date().toLocaleDateString("id-ID", { day: "numeric", month: "long", year: "numeric" }),
      name: selectedBarang,
      nama: selectedBarang,
      jumlah: Number(jumlahKeluar)
    };

    setHistoryKeluar([transaksiBaru, ...historyKeluar]);
    setIsOpen(false);
    setJumlahKeluar("");
  };
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] flex">
      ...
      <div className="flex-1 flex flex-col md:pl-64">
        <Header title="Stok Keluar" />

        <main className="p-6 pb-24 md:pb-6 flex-1 w-full max-w-6xl mx-auto font-['Segoe_UI']">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 mb-6">
            <div>
              <p className="text-sm text-[#6B7280]">Catat pengurangan stok barang.</p>
            </div>
            <button 
              onClick={() => {
                if (daftarBarangMaster.length === 0) {
                  alert("Data master barang kosong! Silakan tambah data di dashboard halaman utama.");
                  return;
                }
                setIsOpen(true);
              }}
              className="w-full sm:w-auto bg-[#EF4444] hover:bg-red-600 text-white text-sm font-semibold px-5 h-10 rounded-xl flex items-center justify-center gap-2 shadow-md transition-all cursor-pointer"
            >
              <span className="text-base font-bold">-</span> Kurangi Stok Barang
            </button>
          </div>
            ...
            <table className="w-full border-collapse text-left text-sm min-w-150">
              <thead className="bg-[#F9FAFB] text-[12px] font-bold tracking-wider text-[#6B7280] uppercase border-b border-[#F3F4F6]">
                <tr>
                  ...
                  <th className="py-4 px-4 w-40 text-center">Stok Keluar</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[#F3F4F6]">
                {historyKeluar.map((item, index) => (
                  <tr key={index} className="hover:bg-slate-50/40 h-16.25 transition-colors">
                    ...
                    <td className="py-3 px-4 text-center">
                      <span className="inline-flex justify-center items-center h-7 px-3 rounded-lg text-sm font-bold bg-[#FEF2F2] text-[#EF4444]">
                        -{item.jumlah}
                      </span>
                    </td>
                  </tr>
            ...
            {historyKeluar.map((item, index) => (
              <div key={index} className="bg-white p-4 rounded-2xl border border-[#F3F4F6] shadow-xs flex flex-col gap-2">
                <div className="flex justify-between items-start">
                  ...
                  <span className="inline-flex justify-center items-center h-6 px-2.5 rounded-lg text-xs font-bold bg-[#FEF2F2] text-[#EF4444]">
                    -{item.jumlah}
                  </span>
                </div>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-xs flex items-center justify-center z-50 p-4">
          <form onSubmit={handleSubmit} className="w-full max-w-md bg-white border border-[#F3F4F6] rounded-2xl shadow-2xl flex flex-col overflow-hidden">
            <div className="p-6 pb-0 flex justify-between items-center">
              <h3 className="text-[20px] font-bold text-[#111827] font-['Segoe_UI']">Catat Stok Keluar</h3>
            ...
            <div className="p-6 flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-sm font-semibold text-[#374151] font-['Segoe_UI']">Jumlah Barang Keluar</label>
                <input 
                  type="number" 
                  value={jumlahKeluar}
            ...
            <div className="bg-[#F9FAFB] p-4 px-6 flex justify-end gap-3 border-t border-gray-50">
              ...
              <button type="submit" className="h-[41.33px] px-5 rounded-xl text-sm font-semibold text-white bg-[#EF4444] hover:bg-red-600 shadow-md cursor-pointer">Kurangi Stok</button>
            </div>
          </form>
        </div>
      )}
