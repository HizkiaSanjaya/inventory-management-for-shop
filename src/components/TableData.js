...
// PERBAIKAN UTAMA: Menambahkan array kosong = [] sebagai tameng cadangan pencegah error length of undefined
    ...
    <div className="w-full flex flex-col font-['Segoe_UI']">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 mb-6">
        <div className="hidden sm:block">
          <p className="text-sm text-[#6B7280]">Kelola semua data barang dan total stok saat ini.</p>
        </div>
        <button 
              ...
              {barangList.length === 0 ? (
                ...
                barangList.map((barang, index) => (
                  <tr key={index} className="hover:bg-slate-50/40 h-16.25 transition-colors">
                    ...
                    <td className="py-3 px-6 font-semibold text-[#111827]">{barang.nama}</td>
                    <td className="py-3 px-4 text-center">
                      <span className={`inline-flex justify-center items-center h-7 px-3 rounded-lg text-sm font-bold ${
                        barang.stok === 0 ? "bg-[#FEF2F2] text-[#DC2626]" : "bg-[#F3F4F6] text-[#374151]"
                      }`}>
                        {barang.stok}
                      </span>
                    </td>
                    <td className="py-3 px-4">
                      <div className="flex justify-center items-center gap-3">
                        <button 
                          onClick={() => onTriggerModal("edit", barang.nama, barang.stok)}
                        ...
                        <button 
                          onClick={() => onTriggerModal("delete", barang.nama)}
      ...
      <div className="flex flex-col sm:hidden gap-3">
        {barangList.length === 0 ? (
          ...
          barangList.map((barang, index) => (
            <div key={index} className="bg-white p-4 rounded-2xl border border-[#F3F4F6] shadow-xs flex flex-col gap-4">
              <div className="flex justify-between items-start gap-2">
                <div className="flex gap-2">
                  <span className="text-xs font-bold text-[#9CA3AF] mt-0.5">#{index + 1}</span>
                  <h4 className="font-semibold text-[#111827] text-sm leading-snug">{barang.nama}</h4>
                </div>
                <span className={`inline-flex shrink-0 justify-center items-center h-7 px-2.5 rounded-lg text-xs font-bold ${
                  barang.stok === 0 ? "bg-[#FEF2F2] text-[#DC2626]" : "bg-[#F3F4F6] text-[#374151]"
                }`}>
                  {barang.stok}
                </span>
              ...
              <div className="flex gap-2 border-t border-gray-50 pt-3">
                <button 
                  onClick={() => onTriggerModal("edit", barang.nama, barang.stok)}
                  className="flex-1 h-9 bg-[#EFF6FF] text-[#3B82F6] flex items-center justify-center gap-1 rounded-lg text-xs font-bold cursor-pointer"
                >
                  ✏️ Edit
                </button>
                <button 
                  onClick={() => onTriggerModal("delete", barang.nama)}
                  className="flex-1 h-9 bg-[#FEF2F2] text-[#EF4444] flex items-center justify-center gap-1 rounded-lg text-xs font-bold cursor-pointer"
                >
                  🗑️ Hapus
                </button>
              </div>
